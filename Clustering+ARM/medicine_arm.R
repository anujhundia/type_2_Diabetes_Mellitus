...
medicine_data <- read.transactions("medicine_arm.csv",
                                   rm.duplicates = FALSE, 
                                   format = "basket",  ##if you use "single" also use cols=c(1,2)
                                   sep=",",  ## csv file
                                   cols=1) ## The dataset HAS row numbers
#inspect(medicine_data)
FrulesK = arules::apriori(medicine_data, parameter = list(support=.5, 
                                                          confidence=.5, minlen=2))
#inspect(FrulesK)
itemFrequencyPlot(medicine_data, topN=20, type="absolute")
